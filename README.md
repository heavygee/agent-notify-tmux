...
**Transcript contract in the wrapper:** `cursor-stop-wrapper.sh` parses `AGENT_NOTIFY_SUMMARY {...}` only from the **latest assistant message** (same JSONL filter as `cursor-done-sound.sh`, over `tail -n $AGENT_NOTIFY_LAST_ASSISTANT_TAIL_LINES`, default 500). It does **not** take the last matching line from the whole transcript, so an earlier official line cannot override later assistant turns. If the latest message has no contract JSON, the wrapper leaves summary fields unset for the done-script LLM path. Set `AGENT_NOTIFY_LAST_ASSISTANT_TAIL_LINES` if your last turn is huge.
